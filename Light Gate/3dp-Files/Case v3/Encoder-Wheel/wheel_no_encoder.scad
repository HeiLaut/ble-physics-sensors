...
module base(){
difference(){
   cuboid([28,62,h],chamfer=2,except=[TOP,BOTTOM],anchor = FRONT);
   cuboid([13,40,h+.2],chamfer=2,except=[TOP,BOTTOM,FRONT],anchor = FRONT);
   translate([0,15,0])xcyl(d= 3.1, h = 80);
   translate([0,50,0])cyl(d=10.5,h = 30);
   translate([0,50,0])rotate([-90,0,0])rodmount(hole=1);
   }
}
...
difference(){
   union(){
      xcyl(d =d,h=5);
      xcyl(d =10,h=6,anchor=LEFT);
      }
   rotate([0,90,0])rotate_extrude()translate([d/2+2.2,0,0])scale([1.5,0.8,1])circle(2.5);
   xcyl(d= 3.5, h = 80);
   *for(i=[0:a:360]){
