    ...
    //holes that fit into the load cell
        translate([versatz,0,0]){
           for(i=[[20,0,-50],[35,0,-50]]){
            ...
            //hole for mount
        translate([-35,0,-20])cylinder(h = 60, d = dLoad+2);   
        }
    }
...
module case(){
    difference(){
        ...
        translate([-27,-y/2+2,1])cuboid([29,w*3,19]);
        //rod hole
        up(z/2-7)rotate([0,90,0])cylinder(d=10.5,h=150);
        //lolin battery and usb cut
         left(lolinpos)fwd(y/2){
             up(1.7)left(7.2)cube([8,w+2,7],center = true);
             right(8.5)cube([11,w+2,5],center=true);
         }
         translate([x/2.5,0,z/2-7])rotate([90,0,0]){
                cylinder(h = y/2+5, d = 5);
               }
         //switch cut
               translate([-x/5,y/2,4])rotate([90,0,0])cylinder(d = 8,h =5);
...
//load cell base
    difference(){
        ...
        //rod hole
        up(z/2-7)right(45)rotate([0,90,0])cylinder(d=10.5,h=50);
